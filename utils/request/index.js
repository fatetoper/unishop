...
fetch.setConfig(config => {
	/* 设置全局配置 */
	config.baseUrl = devConfig.baseUrl; /* 根域名不同 */
	const systemInfo = uni.getSystemInfoSync();
	const systemInfoHeaders = {
		'device-name': systemInfo.brand, // 设备名称
		width: systemInfo.screenWidth, // 屏幕宽度
...
// 刷新refreshToken
async function handleTokenRefreshed(refresh_token) {
	const params = {};
	params.group = mHelper.platformGroupFilter();
	params.refresh_token = refresh_token;
	await fetch.post(refreshToken, params).then(async r => {
		store.commit('login', r.data);
		isRefreshed = false;
	});
}
			...
			case 401:
				...
				if (response.config.url === refreshToken) {
					...
					if (!store.state.refreshToken) {
						...
						// isRefreshed同一个页面只执行一次
						if (!isRefreshed) {
							isRefreshed = true;
							// 刷新token
							await handleTokenRefreshed(store.state.refreshToken, response);
